...
def decide_action(view: dict, can_act: bool, memory_temp: dict = None) -> dict | None:
    ...
    for entry in visible_items_raw:
        ...
        if isinstance(inner, dict):
            ...
            entry["regionId"] = region.get("id", "")
            visible_items.append(entry)

    # Extract region and connections
    region = view.get("currentRegion", {})
    connected_regions = view.get("connectedRegions", [])
    pending_dz = view.get("pendingDeathzones", [])
    
    # Enemy scanning
    enemies_here = visible_agents
    enemies_in_range = [e for e in enemies_here if _is_in_range(e, region, equipped)]
    ...
    # 🚨 EMERGENCY COMBAT DETECTION - Check if we're under attack RIGHT NOW
    # This bypasses all other logic when we're taking damage
    previous_hp = getattr(decide_action, '_last_hp', hp)
    damage_taken = previous_hp - hp
    decide_action._last_hp = hp
    
    # 🚨 PRIORITY PICKUP: Instant pickup for critical items when under attack or in danger
    # This bypasses normal pickup logic for immediate survival
    if items_here and (damage_taken > 0 or hp <= 50 or len(enemies_here) >= 2):
        # Priority 1: Weapons (critical when unarmed)
        if not equipped or equipped.get("typeId", "").lower() == "fist":
            if weapons_here:
                best_weapon = max(weapons_here, key=lambda x: WEAPONS.get(x.get("typeId", "").lower(), {}).get("bonus", 0))
                log.error("⚔️ PRIORITY_WEAPON_PICKUP: Under attack/unarmed - grabbing %s NOW!", best_weapon.get("typeId", "weapon"))
                return {"action": "pickup", "data": {"itemId": best_weapon["id"]},
                        "reason": f"PRIORITY_WEAPON: Under attack, unarmed - immediate pickup NOW!"}
        
        # Priority 2: Healing (critical when HP low)
        if hp <= 40 and healing_here:
            best_heal = max(healing_here, key=lambda x: RECOVERY_ITEMS.get(x.get("typeId", "").lower(), 0))
            log.error("💊 PRIORITY_HEAL_PICKUP: HP=%d low - grabbing %s NOW!", hp, best_heal.get("typeId", "healing"))
            return {"action": "pickup", "data": {"itemId": best_heal["id"]},
                    "reason": f"PRIORITY_HEAL: HP={hp} low, under attack - immediate pickup NOW!"}
        
        # Priority 3: Currency (always valuable)
        if currency_here:
            log.error("💰 PRIORITY_CURRENCY_PICKUP: Under attack - grabbing currency NOW!")
            return {"action": "pickup", "data": {"itemId": currency_here[0]["id"]},
                    "reason": "PRIORITY_CURRENCY: Under attack - immediate pickup NOW!"}
    
    if damage_taken > 0:
        log.error("🚨 EMERGENCY_COMBAT: Taking %d damage! HP: %d→%d. FORCING IMMEDIATE RESPONSE!", damage_taken, previous_hp, hp)
        
        # Emergency flee if HP is critical or overwhelmed
        if hp <= 30 or len(enemies_here) >= 2:
            log.error("🏃 EMERGENCY_FLEE: HP=%d critical or overwhelmed by %d enemies - IMMEDIATE ESCAPE!", hp, len(enemies_here))
            safe = _find_safe_region_with_exit(connected_regions, [], view)
            if safe and ep >= 2:  # Minimum EP to escape
                return {"action": "move", "data": {"regionId": safe},
                        "reason": f"EMERGENCY_FLEE: Taking {damage_taken} damage, HP={hp} critical, escaping NOW!"}
        
        # Emergency heal if we have healing items and HP is low
        if hp <= 40 and healing_here:
            best_heal = max(healing_here, key=lambda x: RECOVERY_ITEMS.get(x.get("typeId", "").lower(), 0))
            log.error("💊 EMERGENCY_HEAL: HP=%d low after taking %d damage - using %s NOW!", hp, best_heal.get("typeId", "healing"))
            return {"action": "use_item", "data": {"itemId": best_heal["id"]},
                    "reason": f"EMERGENCY_HEAL: Taking {damage_taken} damage, HP={hp} critical, healing NOW!"}
        
        # Emergency weapon pickup if unarmed and under attack
        if not equipped or equipped.get("typeId", "").lower() == "fist":
            if weapons_here:
                best_weapon = max(weapons_here, key=lambda x: WEAPONS.get(x.get("typeId", "").lower(), {}).get("bonus", 0))
                log.error("⚔️ EMERGENCY_WEAPON: Under attack, unarmed - picking up %s NOW!", best_weapon.get("typeId", "weapon"))
                return {"action": "pickup", "data": {"itemId": best_weapon["id"]},
                        "reason": f"EMERGENCY_WEAPON: Taking {damage_taken} damage, unarmed - pickup NOW!"}
        
        # Last resort - flee to any available region
        if connected_regions and ep >= 2:
            flee_region = connected_regions[0]  # Take first available
            log.error("🏃 EMERGENCY_ESCAPE: Taking %d damage, no other options - fleeing to %s", flee_region)
            return {"action": "move", "data": {"regionId": flee_region},
                    "reason": f"EMERGENCY_ESCAPE: Taking {damage_taken} damage, last resort escape!"}
